...
PARSE_STATS = {"total": 0, "matched": 0, "failed": 0, "failed_rows": []}
...
class RawDictionaryParser:
    ...
    JYTPNG_RGX = "(?P<jyutping>[a-zA-Z-]*(?:[\u00c0-\u024f]|Ẽ|ẽ|m̃|M̃|M̂|m̂|M̈|m̈|M̄|m̄|M̀|m̀|n̄|N̄|ñ|Ñ|ǹ|Ǹ|n̂|N̂|n̈|N̈)+[^\s]*)"
    PENYIM_RGX = "(?P<penyim>[a-zA-Z-]*[\u00c0-\u024f̈]+[^\s]*)"

    def __init__(
        self, gc_path="../raw/updated_hed.csv", sl_path="../stephen-li.json"
    ) -> None:
        ...
        with open(self.gc_path, "r") as f:
            ...
            for idx, row in enumerate(csv_as_list):
                ...
                if row[3]:
                    ...
                    defn_rgx_pattern = re.compile(
                        f"{self.PTRN_STRT_RGX}{self.SIMP_CH_RGX}?(?:\[{self.TRAD_CH_RGX}\])? {self.JYTPNG_RGX}? ?{self.PENYIM_RGX}? ?"
                    )
                    print(defn.strip())
                    ...
                    # means the word is a composition definition
                        ...
                        r"^\(?(c|C)omposition(( [ts])|( traditional)|( simplified))?: .*\)?",
                        defn,
                    ):
                        continue
                    ...
                    # means the word usage is taishanese specific
                        ...
                        phrase_defn = self.generate_parsed_entry(
                            extracted_tsh_matches, defn, is_taishanese=True
                        )
                        PARSE_STATS["matched"] += 1
