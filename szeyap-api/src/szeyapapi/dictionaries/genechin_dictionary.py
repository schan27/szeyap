...
class GeneChinDictionary(DictionaryBase):
    ...
    def load_dictionary(self):
        with open(
            os.path.join(PROJ_ROOT, GENE_CHIN_DICTIONARY_PATH), "r", encoding="utf-8"
        ) as file:
        ...
        for entry in self.dictionary:
            ...
            entry[SearchLanguage.MANDARIN.value] = entry.get("SIMP", "")
            entry[SearchLanguage.CANTONESE.value] = entry.get("SIMP", "")
            entry[SearchLanguage.TAISHANESE.value] = entry.get("SIMP", "")
        ...
        for i, entry in enumerate(self.dictionary):
            ...
            for word in entry["PENYIM"]:
                if word:
                    try:
                        penyim_obj = Penyim(word, PenyimFormats.GC)
